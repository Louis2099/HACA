...
class PPO:
    ...
    def __init__(
        ...
        if l2c2_cfg is not None:
            self.use_l2c2 = True
            self.lambda_actor = l2c2_cfg.get("lambda_actor", 0.0)
            self.lambda_critic = l2c2_cfg.get("lambda_critic", 0.0)
        else:
            self.use_l2c2 = False
    ...
    def update(self):  # noqa: C901
        ...
        for (
            ...
            if self.update_counter < self.critic_warmup_steps:
                loss = value_loss  # During warmup: only train critic
                ...
                loss = (
                    ...
                    + (surrogate_loss - self.entropy_coef * entropy_batch.mean()) * scale
                )
                self.update_counter += 1
