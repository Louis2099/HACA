...
class OnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        dict_obs = isinstance(obs, TensorDict)

        if dict_obs:
            empty_obs = torch.zeros_like(obs[0])
        else:
            num_obs = obs.shape[1]
        ...
        if dict_obs:
            if self.privileged_obs_type is not None:
                if not isinstance(extras["observations"][self.privileged_obs_type], TensorDict):
                    raise ValueError(
                        f"Privileged observations must be a TensorDict if using a TensorDict observations, got {type(extras['observations'][self.privileged_obs_type])}"
                    )

                empty_privileged_obs = torch.zeros_like(extras["observations"][self.privileged_obs_type][0])

            else:
                empty_privileged_obs = empty_obs
        ...
        if dict_obs:
            # For TensorDict observations, pass the empty TensorDict structure instead of dimensions.
            # This allows the policy network to properly handle dictionary-based observations.
            num_obs, num_privileged_obs = empty_obs, empty_privileged_obs
        policy: (
            ActorCritic | ActorCriticRecurrent | StudentTeacher | StudentTeacherRecurrent | StudentTrainedTeacher
        ...
        # For entropy coefficient annealing
        self.init_entropy_coef = getattr(self.alg, "entropy_coef", 0)
        self.success_episode_length_threshold = (
            self.cfg.get("enable_entropy_coef_annealing_success_rate", 0.0)
            * self.env.unwrapped.max_episode_length
        )
        self.mean_episode_length = 0

    def anneal_entropy_coef(self, progress: float) -> float:
        """Anneal the entropy coefficient based on the progress."""
        if self.cfg.get("enable_entropy_coef_annealing", False):
            if (
                self.mean_episode_length > self.success_episode_length_threshold
                and progress > self.cfg["entropy_coef_annealing_start_progress"]
            ):
                if (decay_rate := self.cfg.get("entropy_annealing_decay_rate", None)) is not None:
                    self.alg.entropy_coef = self.alg.entropy_coef * decay_rate
                else:
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):  # noqa: C901
        ...
        for it in range(start_iter, tot_iter):
            ...
            if hasattr(self.alg, "entropy_coef"):
                self.anneal_entropy_coef(progress)
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35):
        ...
        if len(locs["rewbuffer"]) > 0:
            ...
            # Entropy coefficient
            self.writer.add_scalar(
                "Train/entropy_coef", getattr(self.alg, "entropy_coef",0), locs["it"]
            )
